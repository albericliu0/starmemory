  ...
  if (command === 'sync') {
    ...
    try {
      ...
      if (!background) {
        ...
        console.log(
          `Scanned ${result.filesScanned} files, archived ${result.archived}, indexed ${result.exchangesIndexed} new exchanges${migrated}, ${bm25}, ${result.summarized} summaries written${failed}${expired}.`
        );
      }
    } finally {
      await store.close();
