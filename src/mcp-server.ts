...
server.registerTool(
  'search',
  {
    title: 'Search Memory',
    description:
      'Search past Claude Code and Codex conversations by semantic similarity, exact text, or both. ' +
  ...
  async ({ query, mode, limit, after, before, project, sessionId, harness }) => {
    const text = Array.isArray(query)
      ? formatMultiConceptResults(
          await searchMultipleConcepts(store, index, query, { limit, project, sessionId, harness }),
          query
        )
      : formatResults(
          await search(store, index, query, { mode, limit, after, before, project, sessionId, harness }, textIndex)
        );
