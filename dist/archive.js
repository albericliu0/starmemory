...
export function defaultArchiveRoot(env = process.env) {
...
export function archivePathFor(root, harness, project, sourcePath) {
...
let copyCounter = 0;
export async function copyIfChanged(sourcePath, archivePath) {
    const src = fs.statSync(sourcePath);
    try {
        if (Math.abs(fs.statSync(archivePath).mtimeMs - src.mtimeMs) < 1)
            return false;
    }
    catch {
        // no copy yet
...
export function resolveArchivePath(root, storedPath, harness, project) {
...
export function summaryPathFor(archivePath) {
    let stem = archivePath;
    if (stem.endsWith(ARCHIVE_SUFFIX))
        stem = stem.slice(0, -ARCHIVE_SUFFIX.length);
